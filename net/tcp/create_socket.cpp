...
namespace net {
    namespace tcp {
        ...
        int create_socket(const socket_type type, const std::string& address, const std::uint16_t port) {
            const auto info = get_addrinfo(SOCK_STREAM, address, std::to_string(port));
